...
class DataAnalyzer:
    ...
    def __init__(self, db_path: str = "power_monitoring.db"):
        ...
        # 이상치 통계
        self.outlier_stats = {
            'voltage': OutlierStats(0, 0, 0.0, None, {'mild': 0, 'moderate': 0, 'severe': 0}),
            'current': OutlierStats(0, 0, 0.0, None, {'mild': 0, 'moderate': 0, 'severe': 0}),
            'power': OutlierStats(0, 0, 0.0, None, {'mild': 0, 'moderate': 0, 'severe': 0})
        }
        
        # 최근 분석 결과 (차트 표시용)
        self.recent_results = deque(maxlen=1000)
    
    def analyze_data_point(self, voltage: float, current: float, power: float) -> Dict[str, Any]:
        """단일 데이터 포인트 분석"""
        ...
        for metric, value in [('voltage', voltage), ('current', current), ('power', power)]:
            ...
            stats.outlier_rate = stats.outlier_count / stats.total_samples if stats.total_samples > 0 else 0.0
            ...
            analysis_results[metric] = {
                ...
                'stats': {
                    ...
                    'last_outlier_time': stats.last_outlier_time.isoformat() if stats.last_outlier_time else None
                }
            }
        ...
        overall_result = {
            ...
            'has_any_outlier': any(analysis_results[m]['outlier']['is_outlier'] for m in analysis_results),
            'outlier_count': sum(1 for m in analysis_results if analysis_results[m]['outlier']['is_outlier']),
            'confidence': statistics.mean([analysis_results[m]['outlier']['confidence'] for m in analysis_results])
        }
    ...
    def get_outlier_summary(self) -> Dict[str, Any]:
        ...
        # 전체 통계
        total_samples = sum(stats.total_samples for stats in self.outlier_stats.values())
        total_outliers = sum(stats.outlier_count for stats in self.outlier_stats.values())
        
        summary['overall'] = {
            'total_samples': total_samples,
            'total_outliers': total_outliers,
            'overall_outlier_rate': round((total_outliers / total_samples * 100) if total_samples > 0 else 0, 2),
            'metrics_with_outliers': sum(1 for stats in self.outlier_stats.values() if stats.outlier_count > 0)
        }
