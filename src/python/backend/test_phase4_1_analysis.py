...
class Phase41Tester:
    ...
    async def test_data_analyzer_standalone(self):
        ...
        for voltage, current, power in outlier_data:
            ...
            if result['has_any_outlier']:
                detected_outliers += 1
                print(f"    ✅ Detected outlier: V={voltage}V, A={current}A, W={power}W")
            else:
        ...
        print(f"    Overall outlier rate: {summary['overall']['overall_outlier_rate']}%")
                ...
                while time.time() - start_time < 30:  # 30초 테스트
                    try:
                        ...
                        if data.get("type") == "measurement" and "analysis" in data:
                            ...
                            # 분석 데이터 구조 검증
                            required_fields = ["has_outlier", "outlier_count", "confidence", "moving_averages"]
                            if all(field in analysis for field in required_fields):
                                if analysis["has_outlier"]:
                                    outlier_detected_count += 1
                                    print(f"    🚨 Outlier detected: {analysis['outlier_count']} metrics")
                            
                            if analysis_data_count % 10 == 0:
                                print(f"    📈 Analysis samples: {analysis_data_count}, Outliers: {outlier_detected_count}")
    ...
    async def test_moving_averages_accuracy(self):
        ...
        for metric, avg_dict in averages.items():
            print(f"    {metric}: 1m={avg_dict['1m']:.3f}, 5m={avg_dict['5m']:.3f}, 15m={avg_dict['15m']:.3f}")
        
        # 정확도 검증 (상수 값이므로 모든 이동평균이 동일해야 함)
        voltage_avgs = averages['voltage']
        accuracy_check = (
            abs(voltage_avgs['1m'] - 5.0) < 0.001 and
            abs(voltage_avgs['5m'] - 5.0) < 0.001 and
            abs(voltage_avgs['15m'] - 5.0) < 0.001
        )
    ...
    async def test_outlier_detection_methods(self):
        ...
        for value, should_be_outlier, description in test_cases:
            ...
            print(f"    {description}: Z-score={'✅' if z_correct else '❌'} IQR={'✅' if iqr_correct else '❌'}")
