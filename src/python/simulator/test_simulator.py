...
class SimulatorTester:
    ...
    def run_test(self):
        """테스트 실행"""
        print(f"=== Arduino Simulator Test ===")
    ...
    def _run_command_test(self, sim_manager):
        ...
        commands = [
            ...
            ('{"cmd":"set_mode","value":"LOAD_SPIKE","seq":102}', "Set LOAD_SPIKE mode"),
    ...
    def _run_data_collection_test(self, sim_manager):
        ...
        while time.time() - start_time < self.duration:
            ...
            if current_time - last_print >= 1.0:
                ...
                print(f"  ⏱️ {elapsed:2d}s elapsed, {remaining:2d}s remaining | "
    ...
    def _print_results(self):
        ...
        error_rate = (self.error_count / self.data_count * 100) if self.data_count > 0 else 0
        ...
        if self.voltage_data:
            ...
            print(f"  Voltage: {min(self.voltage_data):.3f}V - {max(self.voltage_data):.3f}V "
                  f"(avg: {sum(self.voltage_data)/len(self.voltage_data):.3f}V)")
            print(f"  Current: {min(self.current_data):.3f}A - {max(self.current_data):.3f}A "
                  f"(avg: {sum(self.current_data)/len(self.current_data):.3f}A)")
            print(f"  Power: {min(self.power_data):.3f}W - {max(self.power_data):.3f}W "
