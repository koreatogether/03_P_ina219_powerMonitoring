...
class SimulatorManager:
    ...
    def _connect_serial(self, port: str) -> bool:
        """시리얼 포트 연결"""
        try:
            self.simulator = SerialSimulator(port, self.config.baudrate, self.config.timeout)
...
# 편의 함수들
def create_simulator(port: str = "AUTO", mock_fallback: bool = True) -> SimulatorManager:
    """시뮬레이터 생성 편의 함수"""
...
def demo_simulator_manager():
    ...
    # 콜백 설정
    def on_data(data):
        print(f"[{sim_manager.get_simulator_type()}] Data: V={data['v']}V, A={data['a']}A, W={data['w']}W")
