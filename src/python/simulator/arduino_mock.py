...
class ArduinoMockSimulator:
    ...
    def _simulation_loop(self):
        ...
        while self.is_running:
            ...
            if current_time - last_mode_change > 30:
                modes = [SimulationMode.NORMAL, SimulationMode.LOAD_SPIKE, 
                        SimulationMode.VOLTAGE_DROP, SimulationMode.NOISE]
                self.current_mode = modes[mode_index % len(modes)]
                mode_index += 1
                last_mode_change = current_time
                self.sensor_status = True
                
                self._send_status_message(f"Auto mode change: {self.current_mode.value}")
    ...
    def _handle_text_command(self, command: str):
        ...
        if cmd == "HELP":
            ...
            self._send_status_message(f"Sensor: {'OK' if self.sensor_status else 'ERROR'}")
            self._send_status_message("=======================")
            ...
            self._send_status_message("Unknown command. Type HELP for available commands.")
